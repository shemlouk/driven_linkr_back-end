...
class HashtagsRepository {
    ...
    async getPostsWithHashtagId(hashtagId) {
        const res = db.query(
            `SELECT u.username, u.profile_picture, p.*, h.name FROM posts p JOIN posts_hashtags ph ON p.id = ph.post_id JOIN hashtags h ON h.id = ph.hashtag_id JOIN users u ON p.user_id = u.id WHERE ph.hashtag_id = $1 ORDER BY created_at DESC;`,
            [hashtagId]);
        return res;
    }
}
